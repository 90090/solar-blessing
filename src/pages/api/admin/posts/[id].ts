/**
 * PATCH  /api/admin/posts/[id]   — approve or reject
...
const apiCall = async (method: string, id: string, body?: object) => {
  ...
  if (!apiUrl) { console.log(`[DEV] ${method} post ${id}`, body); return; }
  const res = await fetch(`${apiUrl}/admin/posts/${encodeURIComponent(id)}`, {
    method,
...
export const PATCH: APIRoute = async ({ request, params }) => {
  ...
  catch (err) { return new Response(JSON.stringify({ message: 'Update failed' }), { status: 500, headers }); }
};
...
export const DELETE: APIRoute = async ({ request, params }) => {
  ...
  catch (err) { return new Response(JSON.stringify({ message: 'Delete failed' }), { status: 500, headers }); }
};
